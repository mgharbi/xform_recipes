...
void MG::smooth(int level) {
    ...
    for(int z = 0 ; z< _len_z ; z++){
        ...
        for(int yo = 0 ; yo < len_y/split ; yo++){
            double v,av;
            for(int yi = 0 ; yi < split ; yi++){
                int y = yo*split+yi;
                if(y >= len_y){
                    break;
                }
                ...
                for (int x=0; x<len_x; x++){
                    ...
                    for(tlist_itr=tlist.begin(); tlist_itr!=tlist.end(); tlist_itr++){
                        if (prev_ind< (*tlist_itr)){
                            tpair.first=&(mP[level][*tlist_itr]);
                            tpair.second=0;		   
                ...
                for (int x=0; x<len_x; x++){
                    ...
                    for (int p=0; p<_perms_num;p++){
                        ...
                        for (i=0; tneb_itr!=end_itr;tneb_itr++, i++){	
                            ...
                            if(  (&mP[level][tneb[i]])==(&mP[level](x,y,z))){
                                tw-=1;
                            }
                            t_data.first=&mP[level][tneb[i]];
