...
int main()
{
    ...
    ImageParam input = ImageParam(type_of<float>(), 3);
    Param<int> nbins;
    Param<int> skip;
    int tile_sz = 32;
    ...
    Func clamped;
    clamped(xi, yi, xo, yo,c) = input(clamp(cast<int>(xo*tile_x + xi*skip),0,input.width()-1),
    ...
    histogram.compile_to_file("hl_histogram_color_float", {input, nbins, skip});
