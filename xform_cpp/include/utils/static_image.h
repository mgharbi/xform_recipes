...
public:
    ...
    Image(T* data, int w, int h, int c) {
        initialize(w,h,c,0);
        int idx = 0;
        for (int z = 0; z < c; ++z)
        for (int y = 0; y < h; ++y)
    ...
    void fuse_channels() {
        ...
        int c = contents->buf.extent[2];
        contents->buf.extent[1] = h*c;
        contents->buf.extent[2] = 0;
        contents->buf.stride[2] = 0;
    }

    void split_channels(int h) {
        assert(dimensions() == 2);
        int w = contents->buf.extent[0];
        int c = contents->buf.extent[1]/h;
        contents->buf.extent[1] = h;
        contents->buf.extent[2] = c;
        contents->buf.stride[2] = h*w;
    }
